      ...
      <script type="text/javascript" src="/Public/sldate/daterangepicker.js"></script>
               ...
               $(document).ready(function() {
                  $('#reservation').daterangepicker(null, function(start, end, label) {
					...
					<form name="admin_list_sea" class="form-search form-horizontal" method="post" action="/index.php/Admin/News/news_back">
...
function del(id,p){
	    ...
	    layer.close(index);
	    window.location.href="/index.php/Admin/News/news_back_del/n_id/"+id+"/p/"+p+"";//重新获取当前页面，删除后返回当前页码
	});
}
function backopen(id,p){
		layer.confirm('你确定要还原文章到文章列表吗？', {icon: 3}, function(index){
	    layer.close(index);
	    window.location.href="/index.php/Admin/News/news_back_open/n_id/"+id+"/p/"+p+"";//重新获取当前页面，删除后返回当前页码
	});
}
